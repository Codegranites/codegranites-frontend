...
const MobileNav = () => {
  ...
  return (
    <>
      ...
      <nav
        className={cn(
          'lg:hidden py-4 sm:py-6 px-4 sm:px-8 xl:px-16 2xl:px-24 flex w-full max-w-[300px] sm:max-w-[70%] md:max-w-[50%] justify-between items-center bg-white/80 backdrop-blur-lg fixed right-0 top-0 z-50 h-screen transition-all opacity-0',
          showMobileMenu ? 'translate-x-0 duration-1000 opacity-100' : 'translate-x-full duration-300',
        )}
      >
        <button
          type="button"
          className="text-primary text-2xl sm:text-4xl absolute top-2 right-2 h-12 w-12 rounded-full border border-primary flex justify-center items-center"
          onClick={() => setShowMobileMenu(false)}
          tabIndex={0}
        >
          <BsX />
